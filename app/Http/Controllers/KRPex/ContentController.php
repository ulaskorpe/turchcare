...
class ContentController extends Controller
{
    ...
    public function list($type,$lang='TR',$parent_id=0){
      ...
      //  dd($type);
       ...
       //  dd($posts->get());
         ...
         return view('admin_panel.content_list',['langs'=>$this->lang_array,'posts'=>$posts->get(),'type'=>$type,'fields'=>$fields,'lang'=>$lang,'parent_id'=>$parent_id,'parent'=>$parent,'parent_title'=>$parent_title,'count'=> (in_array('count',$fields))?true:false]);
    }
    ...
    public function create($type,$lang='TR',$parent_id=0){
        ...
        $parent_cats= [];
        if(in_array('parent_id',$fields)){
            $parent_cats=  $this->findParentCat($type);
        ...
        $count = Post::where('type_id','=',$type['id'])->where('lang','=',$lang)->where('parent_id','=',$parent_id)->count();
        ...
        'fields'=>$fields ,'txt'=>$type['fields'],'langs'=>$this->lang_array,'lang'=>$lang,'count'=>$count,'post'=>null,'exe'=>"Ekle",'route'=>'content-create-post','submit'=>$this->submit_form ,'parent'=>$parent ,'parent_id'=>$parent_id,'cats'=>$cats,'tags'=>$tags,
        'selected_tags'=>[],'video_image_array'=>$this->priority_array($fields)
    ]);
    }
            ...
            if(!in_array($field,$images)){
                    ...
                    if($field=='show_home'){
                        ...
                        $array['youtube_video'] =  $this->youtube_post( $request ['youtube_video']);
                         ...
                         //   dd($array['youtube_video']);
                        }elseif($field == 'parent_id'){
                            $array['parent_id'] =    ( $request['parent_id'] == 'NoCat')?0:$request['parent_id'];
                    }elseif($field == 'faq_id'){
                        if($type['id']==9){
   ...
   //     Log::channel('data_check')->info($request['content']);
        ...
        return  $this->success([''],$type->name." Eklendi" ,201);
    }catch (Exception $e){
    ...
    private function findParentCat(Type $type){

        return Post::where('show_post','=',1)->where('lang','=',session()->get('locale'))
        ->where('type_id','=', Type::where('children','=', $type->slug)->pluck('id')->first())->orderBy('count')->get();
    }
    public function update($id){
       ...
       // dd($type->fields);
        $count = Post::where('type_id','=',$type['id'])->where('lang','=',$post['lang'])->count();
       ...
       // dd($this->priority_array($fields));

        return view('admin_panel.content_form',[ 'type'=>$type ,
        'parent_cats'=>$parent_cats,
        'fields'=>$fields,'txt'=>$type['fields'],'langs'=>$this->lang_array,'lang'=>$post['lang'],'count'=>$count,'post'=>$post,'exe'=>"Güncelle",'route'=>'content-update-post','submit'=>$this->submit_form ,'parent_id'=>$parent_id,'parent'=>$parent,'cats'=>$cats,'slug_link'=>$slug_link,'tags'=>$tags,'selected_tags'=>$selected_tags,'video_image_array'=>$this->priority_array($fields)]);
    }
